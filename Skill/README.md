...
> 先检索证据，再比较模型；从赛题和数据出发，完成求解、算法沉淀、论文写作与 PDF 验收。
...
- 将候选分为 `Baseline`、`Proven` 和 `Frontier`
- 审查更新时间、维护与依赖状态、替代方法及泄漏、复现、许可和安全风险
- 生成 `求解/方法调研.md` 和 `求解/候选方法清单.csv`

### Step 2：求解计划
生成 `求解/求解计划.md`，明确候选来源、公平比较协议、评价方案、文件清单、异常预案和算法沉淀目标。

### Step 3：候选复现与公平比较
- 每问至少包含一个 `Baseline`，通常选择 1–2 个 `Proven`，适用且预算允许时加入一个 `Frontier`
- 统一数据切分、指标、随机种子和调参预算
- 保存候选层级、过时审查结论以及全部成功与失败实验，生成 `模型比较.csv` 和 `模型比较.md`
- 根据实验结果选择主方法，再生成最终图表和结果
