# Axiom Lab 论文模板

> 本目录是 Skill 内置的只读模板副本。初始化比赛工作区时复制这些文件；不要在已安装的 Skill 目录内直接写比赛内容。

---

## 文件清单

| 文件 | 说明 |
|------|------|
| `format.cls` | 论文样式定义文件 |
| `fonts/` | 思源宋体字体（Bold + Regular） |
| `论文.tex` | 主文件（需自行创建，结构见下方） |
| `0.摘要.tex` ~ `10.附录.tex` | 14 个章节空白模板 |

## 章节结构

```
0.摘要.tex           # 通常≤900字且1页；逐问任务—模型—算法—结果
1.引言.tex           # 问题背景 + 问题重述
2.总体分析.tex       # 统一主线、四问难点与递进关系
3.模型假设.tex       # itemize，每条带编号
4.符号说明.tex       # 统一 longtable 表格
5.模型的建立与求解.tex   # 主文件，\input 各问题子文件
5.1.问题1的建立求解.tex  # 每问主文件，\input 两个子文件
5.1.1.分析与准备.tex     # 任务、难点、建模动机与数据准备
5.1.2.建模与求解.tex     # 变量—目标—约束—算法—结果
6.模型检验.tex       # 按题型选择合法性、误差、稳定性或灵敏度
7.模型评价.tex       # 基于验证证据评价优点、限制与适用范围
8.模型改进推广.tex   # 各一段自然段落
9.参考文献.tex       # GB/T 7714-2015，8-15条
10.附录.tex          # 附件说明表
```

## 使用方式

1. 复制本目录下所有文件到工作目录
2. 按 `$solve-math-modeling-competition` 的论文规范创建或编辑 `论文.tex`
3. 编辑各章节 tex 文件填写内容
4. 编译：
   ```bash
   xelatex -interaction=nonstopmode 论文.tex
   xelatex -interaction=nonstopmode 论文.tex
   ```

## 维护约定

- 修改模板请**在此目录**进行，修改后同步至 `Skill/论文/`
- 同步命令（在仓库根目录执行）：
  ```bash
  cp 模板/format.cls Skill/论文/
  cp 模板/*.tex Skill/论文/
  cp -r 模板/fonts Skill/论文/
  ```
